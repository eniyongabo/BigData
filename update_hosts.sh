...
kafka_docker_ip=$(python3 -c 'from __future__ import print_function; import json; c=json.load(open("config.json")); print(c[0]["NetworkSettings"]["Networks"]["kafka-spark-hbase"]["IPAddress"])')
...
hbase_docker_hostname=$(python3 -c 'from __future__ import print_function; import json; c=json.load(open("config.json")); print(c[0]["Config"]["Hostname"])')
hbase_docker_ip=$(python3 -c 'from __future__ import print_function; import json; c=json.load(open("config.json")); print(c[0]["NetworkSettings"]["Networks"]["kafka-spark-hbase"]["IPAddress"])')
...
if grep 'hbase' /etc/hosts >/dev/null; then
  sudo sed -i.bak "s/^.*hbase.*\$/$hbase_docker_ip $hbase_docker_hostname/" /etc/hosts
else
  sudo sh -c "echo '\n$hbase_docker_ip $hbase_docker_hostname' >> /etc/hosts"
fi

echo "connection to host hbase (in the container) enabled"